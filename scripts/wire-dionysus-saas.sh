#!/usr/bin/env bash
# Arm Plutus SaaS pitch enrichment via local Dionysus (:8450).
set -euo pipefail
...
echo "==> Ensure Dionysus is ready"
if [[ -d "$DIO_ROOT/.venv" ]]; then
  cd "$DIO_ROOT"
...
if ! systemctl --user is-active dionysus-homelab >/dev/null 2>&1; then
  if [[ -f "$ROOT/ops/dionysus-homelab-user.service" ]]; then
    ...
    echo "start Dionysus manually on $DIO_URL" >&2
    exit 1
  fi
fi
...
echo "Done — Dionysus pitch armed for SaaS (org=${ORG_SLUG})"